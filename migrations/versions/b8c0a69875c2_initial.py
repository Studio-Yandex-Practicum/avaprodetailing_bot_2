...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            'type', sa.Enum('ACCRUAL', 'WRITE_OFF', name='bonustype'),
            nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            'role', sa.Enum('USER', 'ADMIN', 'SUPERADMIN', name='userrole'),
            nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column(
            'car_body_type', sa.Enum(
                'SEDAN', 'COUPE', 'HATCHBACK', 'LIFTBACK', 'FASTBACK',
                'STATION_WAGON', 'CROSSOVER', 'OFFROAD', 'PICKUP', 'VAN',
                'MINIVAN', 'COMPACT_VAN', 'MICROVAN', 'CONVERTIBLE',
                'ROADSTER', 'TARGA', 'LANDAU', 'LIMOUSINE', name='carbodytype'
            ), nullable=True
        ),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_table(
        ...
        sa.Column(
            'payment_state',
            sa.Enum('WAITING', 'PAID', 'NOT_PAID', name='paymentstate'),
            nullable=False
        ),
        sa.Column('business_unit_id', sa.Integer(), nullable=False),
        sa.Column('user_id', sa.Integer(), nullable=False),
